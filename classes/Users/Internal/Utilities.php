...
class Utilities
{
    ...
    static function getFormSubmitResultHandlerJsCode(): string
    {
        $js = 'var r=event.result;if(r.length>0){' .
            'if(r.substring(0,2)===\'u:\'){clientPackages.get(\'users\').then(function(users){users._closeAllWindows().then(function(){users._showLoading();})});window.location=r.substring(2);}' .
