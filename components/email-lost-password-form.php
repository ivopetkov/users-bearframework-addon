...
$form->onSubmit = function ($values) use ($app, $providerID, $form) {
    ...
    return Utilities::getFormSubmitResult(['jsCode' => 'clientPackages.get("users").then(function(users){users._closeAllWindows().then(function(){users.openProviderScreen("' . $providerID . '","lost-password-email-sent",{"email":"' . $email . '"});})});']);
};
