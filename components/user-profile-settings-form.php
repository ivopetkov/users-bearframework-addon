...
$form->onSubmit = function ($values) use ($app, $providerID, $userID, $getUserData, $form, $hasImage, $hasName, $hasWebsite, $hasDescription) {
    ...
    return Utilities::getFormSubmitResult(['jsCode' => 'clientPackages.get("users").then(function(users){users._updateBadge(' . json_encode(Utilities::getBadgeHTML()) . ');users._closeAllWindows();});']);
};
